...
def plot_results(df, target_bool):
    ...
    ax[0].plot(np.array(hv_iterations["random"]) / global_hv, marker='s', label="random")
    ...
    ax[1].scatter(df[targets[0]], df[targets[1]], alpha=0.5)
    ax[1].scatter(global_pareto_front[:, 0], global_pareto_front[:, 1],
        color='blue', label="Global Pareto Front", marker="*")
    ax[1].scatter(qEHVI_pareto_front[:, 0], qEHVI_pareto_front[:, 1],
        color='red', label="qEHVI Pareto Front", marker="s")
